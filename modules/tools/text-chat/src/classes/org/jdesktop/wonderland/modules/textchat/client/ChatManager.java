...
public class ChatManager implements TextChatListener {
    ...
    /**
     * Singleton to hold instance of ChatManager. This holder class is loaded on
     * the first execution of ChatManager.getChatManager().
     */
    ...
    /**
     * Returns a single instance of this class <p>
     *
    ...
    public void registerConnection(TextChatConnection connection) {
        localUserName = connection.getSession().getUserID().getUsername();
    ...
    /**
     * Creates a new tab in the chat HUD. The panel may exist already and in
     * that case we just create a tab to host the panel. Finally puts the
     * reference to the panel in the user-panel map.
     *
     * @param existingTextChatPanel if null creates a new one, if not it had
     * been created before but the user has closed the tab.
     ...
     * for the first time (If they have opened a direct chat to us). Will be
     * empty in the case that we are opening conversation (or text chat all).
     */
    ...
    /**
     * Unregister and menus we have created, etc. NOTE: (by Design) this is only
     * called on 'Logout' but never on 'Exit' Potential TODO: Register a
     * UserListener to do clean up on 'Exit'
     */
    ...
    /**
     * Creates a new text chat tab, given the remote participants user name and
     * displays it.
     *

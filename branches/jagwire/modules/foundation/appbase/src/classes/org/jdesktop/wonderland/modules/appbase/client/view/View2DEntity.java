...
import org.jdesktop.wonderland.modules.appbase.client.view.scenegraph.ScenegraphChanger;

/**
 * A view which is capable of displaying a window using an MTGame Entity and
 * scene graph. At any one time the window contents can be displayed in the 3D
 * world (
 * <code>setOrtho(false)</code)
 ...
 * <br><br>
 * By default, the view is a quad, but this can be changed via the method
 * <code>setGeometryNode</code>. <br><br> If the user resizable attribute is set
 * to true via
 * <code>setUserResizable</code> this view's dimensions can be dynamically
 * controlled from elsewhere (often via a GUI element) by calling the methods
 * <code>userResizeStart</code>,
 * <code>userResizeUpdate</code> and
 * <code>userResizeFinish</code>. <br><br> A view also supports a number of
 * window manipulation methods which relay actions to the view's window.
 * Examples include windowCloseUser and windowRestackToTop. <br><br> Note: this
 * class supports independent pixel scales for the 3D world and the ortho plane.
 * The values set via
 * <code>setPixelScale</code> are used for the 3D world and the values set via
 * <code>setPixelScaleOrtho</code> are used for the ortho plane. <br><br> This
 * view class also supports a set of positioning methods for when the view is in
 * the ortho plane (e.g.
 * <code>setLocationOrtho</code>) than when the view is in the 3D world (see the
 * <code>View2D</code) class).
 * <br><br>
 * You can move this type of view in the local XY plane when it is in the 3D world by calling
 * <code>applyDeltaTranslationUser</code>. This post-multiplies a translation
 * matrix into the view's current world-to-local transform matrix. Certain
 * utility routines exit (e.g.
 * <code>userMovePlanarStart</code>,
 * <code>userMovePlanarUpdate</code>, and
 * <code>userMovePlanarUpdate</code>) can be called instead in order to make
 * this easier. (Only view's of type SECONDARY can be moved planar). <br><br>
 * TODO: SOMEDAY: Implement move planar for primary views. Must update the cell
 * when doing this. <br><br>
 *
 * @author deronj
 */
@ExperimentalAPI
...
//    };
    ...
    /**
     * The size of the displayed pixels (in local units) when view is in cell
     * mode.
     */
    private Vector2f pixelScaleCell;
    /**
     * The size of the displayed pixels (in local units) when view is in ortho
     * mode.
     */
    ...
    /**
     * The local offset translation from the center of the parent to the center
     * of this view.
     */
    private Vector2f offset = new Vector2f(0f, 0f);
    /**
     * The pixel offset translation from the top left corner of the parent to
     * the top left corner of this view.
     */
    ...
    /**
     * A copy of the current view node's user transformation in world (aka
     * non-ortho) mode.
     */
    protected CellTransform userTransformCell = new CellTransform(null, null);
    /**
     * True if the user transform cell was changed by an entire matrix
     * replacement
     */
    protected boolean userTransformCellReplaced = false;
    /**
     * True if we shouldn't bother informing other clients of modifications to
     * the user transform cell.
     */
    ...
    /**
     * The event listeners which are attached to this view while the view is
     * attached to its cell
     */
    ...
    /**
     * Whether the view entity is to be displayed in ortho mode ("on the
     * glass").
     */
    ...
    /*
     ** TODO: WORKAROUND FOR A WONDERLAND PICKER PROBLEM: * TODO: >>>>>>>> Is
     * this obsolete in 0.5? * * We cannot rely on the x and y values in the
     * intersection info coming from LG * for mouse release events. * The
     * problem is both the same for the X11 and AWT pickers. The LG pickers are
     * * currently defined to set the node info of all events interior to and
     * terminating * a grab to the node info of the button press which started
     * the grab. Not only is * the destination node set (as is proper) but also
     * the x and y intersection info * (which is dubious and, I believe,
     * improper). Note that there is a hack in both * pickers to work around
     * this problem for drag events, which was all LG cared about * at the time.
     * I don't want to perturb the semantics of the LG pickers at this time, *
     * but in the future the intersection info must be dealt with correctly for
     * all * events interior to and terminating a grab. Note that this problem
     * doesn't happen * with button presses, because these start grabs. * * For
     * now I'm simply going to treat the intersection info in button release
     * events * as garbage and supply the proper values by tracking the pointer
     * position myself.
     */
    ...
    // INTRODUCED BY JAGWIRE AS PART OF ISSUE 233
    private List<SceneGraphChange> sceneChanges = Collections.synchronizedList(new LinkedList<SceneGraphChange>());
    private View2DEntityUpdater updater = new View2DEntityUpdater(this, sceneChanges);
     ...
     *
     * NOTE: the subclass must force a complete update after calling this
     * constructor, as follows:
     *
     * changeMask = CHANGED_ALL; update();
     */
    ...
    /**
     * INTERNAL API. <br> Returns the root scene graph node of this view.
     */
    ...
    /**
     * INTERNAL API <br> Disables the GUI2D of this view. Used only by
     * FrameHeaderSwing.
     */
    @InternalAPI
    ...
    /**
     * {@inheritDoc} <br><br> This also has the side effect of setting the ortho
     * attribute of this view (and all of its children) to the ortho attribute
     * of the parent.
     */
    ...
    public synchronized void setVisibleUser(boolean visibleUser, boolean update) {
        if (this.visibleUser == visibleUser) {
            return;
        }
        logger.info("view = " + this);
     ...
     */
    protected synchronized void updateVisibility(boolean inCleanupParent) {
        changeMask |= CHANGED_VISIBLE;
        update();
    ...
    /**
     * Specify the portion of the window which is displayed by this view when it
     * is in cell mode. Update afterward. TODO: notyet: public void
     * setWindowAperture (Rectangle aperture);
     */
    /**
     * Specify the portion of the window which is displayed by this view when it
     * is in cell mode. Update if specified. TODO: notyet: public void
     * setWindowAperture (Rectangle aperture, boolean update);
     */
    /**
     * Return the portion of the window which is displayed by this view when it
     * is in cell mode. TODO: notyet: public Rectangle getWindowAperture ();
     */
    /**
     * Specify the portion of the window which is displayed by this view when it
     * is in ortho mode. Update afterward. TODO: notyet: public void
     * setWindowApertureOrtho (Rectangle aperture);
     */
    /**
     * Specify the portion of the window which is displayed by this view when it
     * is in ortho mode. Update if specified. TODO: notyet: public void
     * setWindowApertureOrtho (Rectangle aperture, boolean update);
     */
    /**
     * Return the portion of the window which is displayed by this view when it
     * is in ortho mode. TODO: notyet: public Rectangle getWindowApertureOrtho
     * ();
     */
    /**
     * {@inheritDoc}
    ...
    public synchronized void setSizeApp(Dimension size, boolean update) {
        if (size.width == sizeApp.width && size.height == sizeApp.height) {
            return;
        }
    ...
    /**
     * Specify the size of the displayed pixels used when the view is in cell
     * mode. Update afterward. This defaults to the initial pixel scale of the
     * window.
     */
    ...
    /**
     * Specify the size of the displayed pixels used when the view is in cell
     * mode. Update if specified. Defaults to the pixel scale of the window.
     */
    ...
    /**
     * Specify the size of the displayed pixels used when the view is in ortho
     * mode. Update afterward. This defaults to (1.0, 1.0).
     */
    ...
    /**
     * Specify the size of the displayed pixels used when the view is in ortho
     * mode. Update if specified. This defaults to (1.0, 1.0).
     */
    ...
    /**
     * Specify the location of a primary view used when the view is in ortho
     * mode. The location is an offset relative to the origin of the displayer
     * and is in the coordinate system of the ortho plane. Update afterward.
     * This attribute is ignored for non-primary views.
     *
     * Note: setPixelOffset is the other part of the ortho offset translation.
     * The two offsets are added to produce the effective offset.
     *
     * Note: there is no corresponding attribute for cell mode because the cell
     * itself automatically controls the location of a primary view within the
     * cell (usually centered) and the cell location within the world is derived
     * from WFS and other input.
     */
    ...
    public synchronized void setLocationOrtho(Vector2f location, boolean update) {
        if (locationOrtho.x == location.x && locationOrtho.y == location.y) {
            return;
        }
        logger.info("view = " + this);
    ...
    /**
     * Issue 151: Returns the corresponding component location for
     * locationOrtho.
     */
    ...
    /**
     * Called for children to mark them as changed when the parent view size
     * changes.
     */
    protected void changeOffsetSelfAndChildren() {
        changeMask |= CHANGED_OFFSET;
    ...
    public synchronized void setPixelOffset(Point pixelOffset, boolean update) {
        if (this.pixelOffset.x == pixelOffset.x && this.pixelOffset.y == pixelOffset.y) {
            return;
        }
        logger.info("view = " + this);
    ...
    /**
     * Called by the UI to indicate a drag vector update during interactive
     * planar move.
     */
    ...
    private void SetWindowVisibilityWherePossible() {
    ...
    private int _3DSizeHasChanged() {
        return changeMask & (CHANGED_SIZE_APP | CHANGED_PIXEL_SCALE);
    }

    private void determineWhichFrameChangesWereMade(ScenegraphChanger changes) {
    ...
    private int frameHasChanged() {
        return changeMask & (CHANGED_DECORATED | CHANGED_TITLE | CHANGED_TYPE | CHANGED_SIZE_APP
                | CHANGED_PIXEL_SCALE | CHANGED_USER_RESIZABLE | CHANGED_VISIBLE);
    }

    private int orthoHasChanged() {
        return changeMask & CHANGED_ORTHO;
    }

    private void processOrthoChanges(ScenegraphChanger changes) {
        ...
        changes.sgChangeViewNodeOrthoSet(viewNode, ortho);
    }

    private void processSizeRelatedChanges(ScenegraphChanger changes) {
        float width = getDisplayerLocalWidth();
        float height = getDisplayerLocalHeight();
        changes.sgChangeGeometrySizeSet(geometryNode, width, height);
    ...
    private boolean processTextureCoordinateChanges(boolean windowNeedsValidate, ScenegraphChanger changes) {
        ...
        if (getWindow() != null && getWindow().getTexture() != null) {
            ...
            changes.sgChangeGeometryTexCoordsSet(geometryNode, widthRatio, heightRatio);
            windowNeedsValidate = true;
        }
        return windowNeedsValidate;
    }

    private boolean processTopologyChanges(boolean windowNeedsValidate, ScenegraphChanger changes) {
        ...
        switch (attachState) {
            case ATTACHED_TO_ENTITY:
                if (parentEntity != null) {
                    ...
                    changes.sgChangeAttachPointSetAddEntity(rc, null, null, null);
        ...
        if ((changeMask & CHANGED_GEOMETRY) != 0) {
            if (geometryNode != null) {
                ...
                changes.sgChangeGeometryDetachFromView(viewNode, geometryNode);
                if (geometrySelfCreated) {
                    changes.sgChangeGeometryCleanup(geometryNode);
                    geometrySelfCreated = false;
                }
            }
            ...
            // Note: don't need to do in RenderUpdater because we've detached our entity
            changes.sgChangeGeometryAttachToView(viewNode, geometryNode);
        }
        // Uses: window
        if ((changeMask & (CHANGED_GEOMETRY | CHANGED_SIZE_APP)) != 0) {
            ...
            if (geometryNode != null) {
                ...
                if (surface != null) {
                    changes.sgChangeGeometryTextureSet(geometryNode, getWindow().getTexture(), surface);
                    windowNeedsValidate = true;
                }
            }
        }
            ...
            if (ortho) {
                ...
                if (type == Type.PRIMARY || type == Type.UNKNOWN) {
                    ...
                    if (parentEntity == null) {
                        ...
                        changes.sgChangeAttachPointSetAddEntity(rc, attachNode, parentEntity, entity);
    ...
    private void processTransformationRelatedChanges(ScenegraphChanger changes) {
        ...
        changes.sgChangeGeometryTransformOffsetStackSet(geometryNode, transform);
    }

    private void processZOrderChanges(ScenegraphChanger changes) {
        ...
        if (ortho) {
            if (window != null) {
                ...
                if (zOrder >= 0) {
                    changes.sgChangeGeometryOrthoZOrderSet(geometryNode, zOrder);
                }
            }
        }
    }

    private int sizeHasChanged() {
        return changeMask & (CHANGED_DECORATED | CHANGED_SIZE_APP | CHANGED_PIXEL_SCALE
                | CHANGED_ORTHO);
    }

    private int textureCoordinatesHaveChanged() {
        return changeMask & (CHANGED_TEX_COORDS | CHANGED_GEOMETRY | CHANGED_SIZE_APP);
    }

    private int topologyHasChanged() {
        return changeMask & (CHANGED_GEOMETRY | CHANGED_SIZE_APP | CHANGED_TYPE | CHANGED_PARENT
                | CHANGED_VISIBLE | CHANGED_ORTHO);
    }

    private int transformationHasChanged() {
        return changeMask & (CHANGED_TYPE | CHANGED_PARENT | CHANGED_PIXEL_SCALE | CHANGED_SIZE_APP
                | CHANGED_OFFSET | CHANGED_ORTHO | CHANGED_LOCATION_ORTHO | CHANGED_STACK);
    }
    ...
    private void updateOffsetsOfChildren() {
        for (View2DEntity childView : children) {
            childView.changeOffsetSelfAndChildren();
        }
    }

    private void processViewNodeChanges(ScenegraphChanger changes) {
        ...
        switch (type) {
            ...
            case PRIMARY:
                ...
                break;
            case SECONDARY:
                changes.sgChangeTransformUserSet(viewNode, currentUserTransform);
            ...
            case POPUP:
                ...
                changes.sgChangeTransformUserSet(viewNode, new CellTransform(null, null));
        }
    ...
    /**
     * Called by the UI to indicate a drag vector update during interactive
     * resize. This is called on the EDT.
     */
    ...
    /**
     * TODO: NOTYET public synchronized void userMoveZStart (float dy) { }
     *
     ...
     */
    /**
     * Specifies whether the view entity is to be displayed in ortho mode ("on
     * the glass"). Update immediately. In addition, the ortho attribute of all
     * descendents is set to the same value. And, when ortho is true, the
     * decorated attribute is ignored and is regarded as false.
     */
    ...
    /**
     * Specifies whether the view entity is to be displayed in ortho mode ("on
     * the glass"). Update if specified. This also changes the ortho attribute
     * of all descendent views.
     */
    ...
    public LinkedList<FrameChange> getFrameChanges() {
    ...
    public boolean isGeometrySelfCreated() {
        return geometrySelfCreated;
    }

    public void setGeometrySelfCreated(boolean selfCreated) {
        geometrySelfCreated = selfCreated;
    }

    public LinkedList<View2DEntity> getChildren() {
        return children;
    }
...
//        l.warning("USING UPDATE_PASS_1!");
     ...
     * {@inheritDoc}
     */
//    public synchronized void legacy_update() {
//
////        updater.update();
//
//        
//        
//        
//        //<editor-fold defaultstate="collapsed" desc="LEGACY UPDATE METHOD">
...
//        
//    }
    public synchronized void update_pass_1() {
        //<editor-fold defaultstate="collapsed" desc="LEGACY UPDATE METHOD">
        // Only-Update-When-Visible Optimization
        // 1. Always perform any visibility or size changes immediately.
        if (visibilityHasChanged() == 0) {
            // 2. Don't perform other changes unless the view if both the app and the user
            // have made it visible.
            if (!visibleApp || !visibleUser) {
                return;
        ...
        // Note: all of the scene graph changes are queued up and executed at the end
        boolean windowNeedsValidate = false;
        ScenegraphChanger changer = new ScenegraphChanger(this);

        // React to topology related changes
        if (topologyHasChanged() != 0) {
            windowNeedsValidate = processTopologyChanges(windowNeedsValidate, changer);
        }

        // Determine what frame changes need to be performed. But these aren't executed now;
        // they are executed later by view.updateFrame, which must be invoked outside the window lock.
        // issue 151: prepare for reattachment of frame on resize.
        if (frameHasChanged() != 0) {
            determineWhichFrameChangesWereMade(changer);
        }

        if (zOrderHasChanged() != 0) {
            processZOrderChanges(changer);
        }

        if (orthoHasChanged() != 0) {
            processOrthoChanges(changer);
        }

        // React to size related changes (must be done before handling transform changes)
        if (sizeHasChanged() != 0) {
            processSizeRelatedChanges(changer);
        }

        // React to texture coordinate changes
        // Uses: window, texture
        if (textureCoordinatesHaveChanged() != 0) {
            windowNeedsValidate = processTextureCoordinateChanges(windowNeedsValidate, changer);
        }

        // React to transform related changes
        // Uses: type, parent, pixelscale, size, offset, ortho, locationOrtho, stack
        if (transformationHasChanged() != 0) {
            processTransformationRelatedChanges(changer);
        }

        // Update the view node's user transform, if necessary
        // Uses: type, deltaTranslationToApply
        if (viewNodeHasChanged() != 0) {
            processViewNodeChanges(changer);
        }

        // Changing the 3D size of the app can change the offset of children, such as headers.
        if (_3DSizeHasChanged() != 0) {
            updateOffsetsOfChildren();
        }

//        sgProcessChanges(changer);
        changer.processChanges();

        validateWindowIfNecessary(windowNeedsValidate);

        SetWindowVisibilityWherePossible();
    ...
    private void validateWindowIfNecessary(boolean windowNeedsValidate) {
        // In certain situations, especially after we change the texture, WindowSwings
        // need to be repainted into that texture.
        if (windowNeedsValidate) {
            if (window instanceof WindowSwing) {
    ...
    private int visibilityHasChanged() {
        return changeMask & (CHANGED_VISIBLE | CHANGED_SIZE_APP);
    }

    private int zOrderHasChanged() {
        return changeMask & (CHANGED_STACK | CHANGED_ORTHO);
    }

    private int viewNodeHasChanged() {
        return changeMask & (CHANGED_TYPE | CHANGED_USER_TRANSFORM | CHANGED_ORTHO);
    }

    /**
     * This returns the entity in the world to which this view should be
     * attached as a child. It must return non-null. Uses: type, parent.
     */
    ...
    //<editor-fold defaultstate="collapsed" desc="sgchange stuph">
    ...
    private synchronized void sgChangeGeometryAttachToView(List<SGChange> sgChanges, Node viewNode, GeometryNode geometryNode) {
        sgChanges.add(new SGChangeGeometryAttachToView(viewNode, geometryNode));
    }

    private synchronized void sgChangeGeometryDetachFromView(List<SGChange> sgChanges, Node viewNode, GeometryNode geometryNode) {
        sgChanges.add(new SGChangeGeometryDetachFromView(viewNode, geometryNode));
    }

    private synchronized void sgChangeGeometrySizeSet(List<SGChange> sgChanges, GeometryNode geometryNode, float width, float height) {
        sgChanges.add(new SGChangeGeometrySizeSet(geometryNode, width, height));
    }

    private synchronized void sgChangeGeometryTexCoordsSet(List<SGChange> sgChanges, GeometryNode geometryNode, float widthRatio,
            float heightRatio) {
        sgChanges.add(new SGChangeGeometryTexCoordsSet(geometryNode, widthRatio, heightRatio));
    }

    private synchronized void sgChangeGeometryTextureSet(List<SGChange> sgChanges, GeometryNode geometryNode, Texture2D texture,
            DrawingSurface surface) {
        sgChanges.add(new SGChangeGeometryTextureSet(geometryNode, texture, surface));
    }

    private synchronized void sgChangeGeometryOrthoZOrderSet(List<SGChange> sgChanges, GeometryNode geometryNode, int zOrder) {
        sgChanges.add(new SGChangeGeometryOrthoZOrderSet(geometryNode, zOrder));
    }

    private synchronized void sgChangeGeometryCleanup(List<SGChange> sgChanges, GeometryNode geometryNode) {
        sgChanges.add(new SGChangeGeometryCleanup(geometryNode));
    }

    private synchronized void sgChangeViewNodeOrthoSet(List<SGChange> sgChanges, Node viewNode, boolean ortho) {
        sgChanges.add(new SGChangeViewNodeOrthoSet(viewNode, ortho));
    }

    private synchronized void sgChangeGeometryTransformOffsetStackSet(List<SGChange> sgChanges, GeometryNode geometryNode,
            CellTransform transform) {
        sgChanges.add(new SGChangeGeometryTransformOffsetStackSet(geometryNode, transform));
    }

    protected synchronized void sgChangeTransformUserSet(List<SGChange> sgChanges, Node viewNode, CellTransform transform) {
        sgChanges.add(new SGChangeTransformUserSet(viewNode, transform));
    }

    protected synchronized void sgChangeAttachPointSetAddEntity(List<SGChange> sgChanges, RenderComponent rc, Node node,
    ...
    //<editor-fold defaultstate="collapsed" desc="sgProcessChanges methods">
    ...
    private void sgProcessChanges(final List<SGChange> sgChanges) {
        if (sgChanges.size() <= 0) {
            return;
        }
     ...
     *
     * @param inRenderLoop true if the call is already being made from inside
     * the render loop.
     */
    public void forceTextureIdAssignment(boolean inRenderLoop) {
        if (geometryNode == null) {
        ...
        /*
         * For debug: Verify that ID was actually allocated Texture tex =
         * geometryNode.getTexture(); if (tex != null) { int texid =
         * tex.getTextureId(); logger.severe("))))))))))) V2E: texid alloc:
         * texid = " + texid); if (texid == 0) { logger.severe("Failed to
    ...
    /**
     * Detach frame from the view and reattach it. This takes into account any
     * changes in the type of the view. This can be overridden by the subclass
     * so a subclass-specific frame can be reattached.
     */
    ...
    /**
     * Make an entity pickable by attaching a collision component. Entity must
     * already have a render component and a scene root node.
     */
    ...
    protected float getHackZEpsilon() {
        return hackZEpsilon;
    }

    protected Vector3f getDeltaTranslationToApply() {
        return this.deltaTranslationToApply;
    }
}

...
@ExperimentalAPI
public class ImageViewerApp extends AppGraphics2D  {
    
    /** The single window created by this app */
    private ImageViewerWindow window;

    /**
     * Create a new instance of ImageViewerApp. This in turn creates
     ...
     */
    public ImageViewerApp (AppType appType, int width, int height, Vector2f pixelScale) {
	...
	// This app has only one window, so it is always top-level 
        try {
            window = new ImageViewerWindow(this, width, height, true, pixelScale);
     ...
     */
    public ImageViewerWindow getWindow () {
	return window;
    }

...
class TextTextureGenerator {
    ...
    private void printBufImage (BufferedImage bi) {
	int w = bi.getWidth();
	int h = bi.getHeight();
	...
	//System.err.println("texWH = " + texWidth + " " + texHeight);
	int biWidth = getRoundUptoPow2(texWidth);
	int biHeight = getRoundUptoPow2(texHeight);
	//System.err.println("biWH = " + biWidth + " " + biHeight);
	...
	System.err.println("text (unclipped) = " + text);
	text = clipText(text, hScale, texWidth, g2d);
	System.err.println("text (final clipped) = " + text);

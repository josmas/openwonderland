 ...
 * (<code>pickKeyEventNonSwing</code>) is used in both the Swing and non Swing cases.
 *
...
public abstract class InputPicker {
    ...
    /**
     * Process key events. No picking is performed. Key events are delivered starting at the
	...
	// from least to greatest eye distance.
        PickInfo hitPickInfo = pickEventScreenPos(event.getX(), event.getY());
	...
	Vector3f eyePos = camera.getLocation();
	...
	// Note: pickAll is needed to in order to pick through transparent objects
        return colisionSys.pickAllWorldRay(pickRayWorld, cameraComp, true, false/*TODO:interp*/)
    }
